...
namespace LMS
{
    ...
    public class Client : User
    {
        ...
        public void requestABook()
        {
            ...
            UI.ShowSuccess("We have gotten your request, the book will be added to the library as soon as possiblie");
        ...
        public void borrowBook()
        {
            if (restrictedClients.ContainsKey(this.userID))
            {
                ...
                logs.Add(new Log($"Borrowed Book", this.userID, false));
                return;
            }
            ...
            if (books.ContainsKey(bookID))
            {
                ...
                {
                    UI.Load(times: new Random().Next(50, 70), displayMsg: "Getting Book...", sequenceCode: 1);
                    UI.ProgressBar(displayMsg: "Updating Your Book Inventory....", interval: new Random().Next(50, 100));
                    
                    borrowedBooks.Add(bookID, books[bookID]);
                    BorrowedBooks.Add(books[bookID]);
                    books[bookID].bookBorrowed(dueDays);
                    UI.ShowSuccess($"You have borrowed {books[bookID].name} successfully, \nNice choice, I hope you enjoy reading this ;)");
        ...
        public void returnBook()
        {
            ...
            {
                UI.throwError($"{this.firstName}, this book is not in your possession\n");
                logs.Add(new Log($"Returned Book: {bookID}", this.userID, false));
                return;
            }
        }
        ...
        public void listBooksInPossession()
        {
            if (borrowedBooks != null)
            {
                ...
                UI.throwError($"{this. firstName}, you currently do not have any book in your possession");
                logs.Add(new Log($"Client {this.userID} Viewed Books In Possession", this.userID, true));
                return;
            }
        }
